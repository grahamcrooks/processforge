...
async def generate_bpmn_artefacts(
    ...
    """BPMN Generation mode — all diagrams stitched into one Blueprint-ready BPMN + combined DDL + OpenAPI."""
    image_exts = {".png", ".jpg", ".jpeg", ".gif", ".webp"}
    for f in diagrams:
        ext = ("." + f.filename.rsplit(".", 1)[-1].lower()) if f.filename and "." in f.filename else ""
        is_image = (f.content_type and f.content_type.startswith("image/")) or ext in image_exts
    ...
    async def event_stream():
        try:
            ...
            # ── Phase 2: generate combined artefacts ──────────────────────
            yield _sse({"type": "step", "key": "bpmn"})
            files.append(encode_file("combined.bpmn", generate_bpmn(process_data), "application/xml"))
            yield _sse({"type": "detail", "message":
                f"combined.bpmn — {steps_count} steps across {lanes_count} lanes (ready for Blueprint import)"})

            yield _sse({"type": "step", "key": "ddl"})
            files.append(encode_file("combined-schema.sql", generate_ddl(process_data, None), "text/plain"))
            yield _sse({"type": "detail", "message":
                f"combined-schema.sql — {len(entities)} {'entity' if len(entities) == 1 else 'entities'}"})

            yield _sse({"type": "step", "key": "openapi"})
            files.append(encode_file("combined-api-spec.yaml", generate_openapi(process_data), "application/x-yaml"))
            yield _sse({"type": "detail", "message":
            ...
            yield _sse({"type": "detail", "message":
                f"Packaged {len(files)} artefacts — 1 stitched BPMN + combined DDL + OpenAPI"})
